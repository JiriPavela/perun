...
def extract_configuration(engine, probes):
    ...
    extracted_func.update(probes.func)
    ...
    # Assign unique ID and sampling index to every probe
    _add_probe_ids(probes)
...
def _add_probe_ids(probes):
    ...
    for probe_collection in [probes.func, probes.usdt]:
        for probe in sorted(probe_collection.values(), key=lambda value: value['name']):
            ...
            if probe['sample'] > 1:
                ...
                probes.register_sampled_probe(probe['name'], probe['type'])
